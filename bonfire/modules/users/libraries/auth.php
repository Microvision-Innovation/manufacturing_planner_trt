...
class Auth
{
	...
	public function login($login, $password, $remember=FALSE)
	{
		...
		if ($user == FALSE)
		{
			Template::set_message(lang('us_bad_email_pass'), 'alert fresh-color alert-danger');
			return FALSE;
		}
		...
		if ($user->active == 0 && $activation_type > 0) // in case we go to a unix timestamp later, this will still work.
		{
			...
			{
				Template::set_message(lang('us_account_not_active'), 'alert fresh-color alert-danger');
			}
			elseif ($activation_type == 2)
			{
				Template::set_message(lang('us_admin_approval_pending'), 'alert fresh-color alert-danger');
			}
		...
		if ($this->check_password($password, $user->password_hash))
		{
			...
			if ($user->banned)
			{
				...
				Template::set_message($user->ban_message ? $user->ban_message : lang('us_banned_msg'), 'alert fresh-color alert-danger');
				return FALSE;
			}
            ...
            {
                Template::set_message(lang('us_forced_password_reset_note'), 'alert fresh-color alert-warning');
	...
	public function restrict($permission=NULL, $uri=NULL)
	{
		...
		if ($this->is_logged_in() === FALSE)
		{
			...
			Template::set_message($this->ci->lang->line('us_must_login'), 'alert fresh-color alert-warning');
